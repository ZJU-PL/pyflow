Store Graph Intermediate Representation
=======================================

The store graph (``pyflow.ir.storegraph``) is the **foundational data
...
.. list-table::
   ...
   * - Module
     - How it uses the store graph
   * - ``cpa``
     - Constraint propagation over points-to relationships
   * - ``ipa``
     - Interprocedural analysis across store graph regions
   * - ``shape``
     - Region-based shape inference using CPA results
   * - ``lifetimeanalysis``
     - Read/modify tracking using shape and CPA results
   * - ``alias/flow_sensitive``
     - Independent subsystem operating on the CFG supergraph; it does not use
       the store graph

.. note::

   The :doc:`/analysis/alias/flow_sensitive` analysis uses its own model (``HeapLocation``,
