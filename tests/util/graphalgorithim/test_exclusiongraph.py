...
from pyflow.util.graphalgorithim import exclusiongraph
...
class TestExclusionGraph(unittest.TestCase):
    ...
    def assertExclusive(self, *args):
        self.assertTrue(
            self.exg.mutuallyExclusive(*args),
            "%s should be mutually exclusive" % self.argString(args),
        )

    def assertNotExclusive(self, *args):
        self.assertFalse(
            self.exg.mutuallyExclusive(*args),
            "%s should not be mutually exclusive" % self.argString(args),
        )
