...
from tests.analysis.shape.shape_base import *

import pyflow.analysis.shape.transferfunctions
...
class TestExpressions(TestConstraintBase):
    def shapeSetUp(self):
        ...
        self.allExpr = set(
            (self.zero, self.zero_a, self.zero_a_b, self.one, self.one_a, self.one_a_b)
        )
    ...
    def testUpdateHitMiss1(self):
        ...
        newPaths = analysis.shape.transferfunctions.updateHitMiss(
            self.sys, e0, e1, b0, b1, slot, paths
        )
...
class TestReferenceCounts(TestConstraintBase):
    ...
    def testIncrementSaturate(self):
        ...
        for i in range(self.sys.canonical.rcm.k + 1):
            ...
            for cslot, count in next.counts.items():
...
class TestLocalAssignConstraint(TestConstraintBase):
    def shapeSetUp(self):
        ...
        # b = a
        self.setConstraint(
            analysis.shape.constraints.AssignmentConstraint(
                self.sys, self.inputPoint, self.outputPoint, self.aExpr, self.bExpr
            )
        )
...
class TestLoadAssignConstraint(TestConstraintBase):
    def shapeSetUp(self):
        ...
        self.setConstraint(
            analysis.shape.constraints.AssignmentConstraint(
                self.sys, self.inputPoint, self.outputPoint, self.axExpr, self.bExpr
            )
        )
...
class TestStoreAssignConstraint(TestConstraintBase):
    def shapeSetUp(self):
        ...
        self.setConstraint(
            analysis.shape.constraints.AssignmentConstraint(
                self.sys, self.inputPoint, self.outputPoint, self.aExpr, self.bxExpr
            )
        )
