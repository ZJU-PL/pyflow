...
class TestSimpleCase(TestCompoundConstraintBase):
    def shapeSetUp(self):
        ...
        self.body = ast.Suite([ast.Assign(y, [z]), loop, ast.Return([z])])
...
class TestRecursiveCase(TestCompoundConstraintBase):
    ...
    def shapeSetUp(self):
        ...
        self.headSlot = self.sys.canonical.fieldSlot(
            None, ("LowLevel", self.extractor.getObject("head"))
        )
        self.tailSlot = self.sys.canonical.fieldSlot(
            None, ("LowLevel", self.extractor.getObject("tail"))
        )
        ...
        temp = ast.Local("temp")
        tb = ast.Suite([ast.Assign(dc, [temp]), ast.Return([temp])])
