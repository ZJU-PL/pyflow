...
import pyflow.analysis.shape.model.canonical as canonical
import pyflow.analysis.shape.model.pathinformation as pathinformation
...
class TestPathInformation(TestPathInformationBase):
    ...
    def testHits2(self):
        ...
        self.assertTrue(info.mustAlias(self.a, self.b))
        self.assertTrue(info.mustAlias(self.an, self.bn))
    ...
    def testLoop(self):
        ...
        self.assertTrue(not info.mustAlias(self.a, self.b))
        self.assertTrue(not info.mustAlias(self.an, self.bn))

        self.assertTrue(info.mustAlias(self.a, self.an))
        self.assertTrue(info.mustAlias(self.an, self.ann))
    ...
    def testTricky1(self):
        ...
        self.assertTrue(not info.mustAlias(self.a, self.b))
        self.assertTrue(info.mustAlias(self.b, self.c))
        self.assertTrue(not info.mustAlias(self.c, self.d))

        self.assertTrue(info.mustAlias(self.an, self.bn))
        self.assertTrue(info.mustAlias(self.an, self.cn))
        self.assertTrue(info.mustAlias(self.an, self.dn))
        self.assertTrue(info.mustAlias(self.bn, self.cn))
        self.assertTrue(info.mustAlias(self.bn, self.dn))
        self.assertTrue(info.mustAlias(self.cn, self.dn))
    ...
    def testTricky2(self):
        ...
        self.assertTrue(info.mustAlias(self.an, self.ann))
        self.assertTrue(info.mustAlias(self.bn, self.bnn))
        self.assertTrue(info.mustAlias(self.cn, self.cnn))
        self.assertTrue(info.mustAlias(self.dn, self.dnn))

        self.assertTrue(info.mustAlias(self.an, self.bnn))

    def testIntersect1(self):
        info1 = pathinformation.PathInformation()
        info1.union(self.a, self.an)
        self.assertTrue(info1.mustAlias(self.a, self.an))
        self.assertTrue(info1.mustAlias(self.a, self.ann))

        info2 = pathinformation.PathInformation()
        info2.union(self.a, self.ann)
        self.assertTrue(not info2.mustAlias(self.a, self.an))
        self.assertTrue(info2.mustAlias(self.a, self.ann))

        info3, changed = info1.inplaceMerge(info2)

        self.assertTrue(changed)
        self.assertTrue(not info3.mustAlias(self.a, self.an))
        self.assertTrue(info3.mustAlias(self.a, self.ann))
...
class TestPathInfoSplit(PathInfoBase):
    ...
    def testExtendParameters(self):
        ...
        self.assertTrue(self.paths.mustAlias(self.etr, self.tr))
        self.assertTrue(self.paths.mustAlias(self.etr, self.yr))
        self.assertTrue(self.paths.mustAlias(self.etr, self.zf))
    ...
    def testSplit(self):
        ...
        self.assertTrue(self.paths.mustAlias(self.yr, self.zf))

        accessed, hidden = self.paths.split(self.extendedParams, accessedCallback)

        self.assertTrue(accessed.mustAlias(self.tr, self.etr))
        self.assertTrue(accessed.mustAlias(self.trl, self.etrl))
        self.assertTrue(not accessed.mustAlias(self.y, self.et))

        self.assertTrue(not accessed.mustAlias(self.yr, self.zf))

        self.assertTrue(hidden.mustAlias(self.zf, self.etr))
        self.assertTrue(hidden.mustAlias(self.xf, self.etrl))
        self.assertTrue(hidden.mustAlias(self.y, self.et))

        self.assertTrue(not hidden.mustAlias(self.yr, self.zf))
...
class TestUglyPathInfoSplit(PathInfoBase):
    ...
    def testHits(self):
        ...
        self.assertTrue(self.paths.mustAlias(self.xlr, self.yrr))
    ...
    def testSplit(self):
        ...
        self.assertTrue(not accessed.mustAlias(self.xlr, self.yrr))

        self.assertEqual(hidden.hit(self.xlrr), TVLMaybe)
        self.assertEqual(hidden.hit(self.yrrr), TVLTrue)
        self.assertTrue(not hidden.mustAlias(self.xlr, self.yrr))
