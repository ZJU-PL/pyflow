...
import pyflow.analysis.programextractor
...
class TestConstraintBase(unittest.TestCase):
    def setUp(self):
        ...
        self.extractor = pyflow.analysis.programextractor.Extractor(compiler)
    ...
    def convert(self, row, entry):
        ...
        allocated = False
        index = self.sys.canonical.configuration(
    ...
    def match(self, entrySet, point, index):
        return (
            ...
            and index.entrySet == entrySet
            and not index.externalReferences
        )

    def countOutputs(self, entrySet):
        count = 0
        for point, context, index in self.sys.environment._secondary.keys():
    ...
    def dumpOutputs(self, entrySet):
        ...
        for (
            ...
            context,
            index,
        ), secondary in self.sys.environment._secondary.items():
            if self.match(entrySet, point, index):
                print(">" * 40)
        ...
        try:
            for row in results:
                ...
                if misses:
                    for e in misses:
                        self.assertEqual(
                            secondary.paths.hit(e), TVLFalse, "%r should be a miss." % e
                        )

                if unknowns:
                    for e in unknowns:
                        self.assertEqual(
                            secondary.paths.hit(e),
                            TVLMaybe,
                            "%r should be unknown." % e,
                        )
    ...
    def dumpPoint(self, givenPoint):
        ...
        for (point, context, conf), secondary in mapping.items():
            if point != givenPoint:
                continue
