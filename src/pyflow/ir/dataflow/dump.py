...
import pyflow.ir.dataflow.graph as graph
...
class DataflowToDot(TypeDispatcher):
    ...
    def compoundStyle(self, node):
        ...
        return dict(
            ...
            shape="box",
            style="filled",
            fillcolor=self.style.compoundColor,
            fontsize=8,
        )
    ...
    def edge(self, src, dst, style="solid"):
        ...
        if not self.showNull and slot.isNull():
            return
    ...
    def handleGenericOp(self, node):
        # In
        for slot in node.localReads.values():
            assert slot.isUse(node)
            self.edge(slot, node)

        for slot in node.heapReads.values():
            assert slot.isUse(node)
            self.edge(slot, node)

        for slot in node.heapPsedoReads.values():
            assert slot.isUse(node)
        ...
        for slot in node.heapModifies.values():
            assert slot.isDefn(node)
            self.edge(node, slot)
            self.mark(slot)
    ...
    def process(self, dataflow):
        ...
        for e in dataflow.existing.values():
            self.mark(e)
...
class ClusterNodes(object):
    ...
    def process(self, dataflow):
        pending = set(dataflow.entry.modifies.values())
        processed = set()

        self.handleGroup(dataflow.entry.modifies.values())
    ...
    def dump(self):
        for node, cluster in self.cluster.lut.items():
