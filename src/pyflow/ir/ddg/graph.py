...
Graphs. The DDG is built from dataflow IR graphs and represents data dependencies
between operations.

**Node Types:**
- **Op nodes**: Represent operations from dataflow IR (e.g., GenericOp, Merge, Split)
...
traversal in both directions. Nodes are indexed by their dataflow IR counterparts
...
class DDGNode(object):
    ...
    A DDG node wraps a dataflow IR node (either an OpNode or SlotNode) and
    tracks its data dependencies through incoming and outgoing edges.

    **Node Categories:**
    - "op": Operation node (wraps dataflow IR OpNode)
    - "slot": Slot node (wraps dataflow IR SlotNode)
    ...
    **Bidirectional Edges:**
    ...
    Attributes:
        ...
        ir_node: The underlying dataflow IR node (OpNode or SlotNode)
    ...
    def __init__(
        ...
        Args:
            ...
            ir_node: The underlying dataflow IR node
...
class DataDependenceGraph(object):
    ...
    a dataflow IR graph. It maintains mappings from dataflow IR nodes to DDG nodes
    ...
    **Node Management:**
    ...
    allowing efficient lookup by dataflow IR node. Nodes are created on-demand
    and cached to ensure each dataflow IR node has a unique DDG node.
    ...
    **Graph Statistics:**
    ...
    Attributes:
        ...
        op_node_map: Mapping from dataflow IR OpNode to DDGNode
    ...
    def get_or_create_op_node(self, ir_op: Any) -> DDGNode:
        """
        Get or create a DDG node for a dataflow IR operation.
    ...
    def get_or_create_slot_node(self, ir_slot: Any) -> DDGNode:
        """
        Get or create a DDG node for a dataflow IR slot.
