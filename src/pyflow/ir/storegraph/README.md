...
`pyflow.ir.storegraph` defines the heap/state model used by the CPA and IPA layers. It gives the analysis a uniform way to talk about:
...
`CanonicalObjects` in [`canonicalobjects.py`](canonicalobjects.py) creates:
...
The concrete variants in [`extendedtypes.py`](extendedtypes.py) are:
...
This is the bridge between the store graph and the CPA constraint system in [`constraints.py`](../../analysis/cpa/constraints.py).

## Set Management

[`setmanager.py`](setmanager.py) provides `CachedSetManager`, a small helper that interns `frozenset` results. The graph uses it for nearly every points-to set operation:
...
[`annotations.py`](annotations.py) defines immutable metadata objects attached directly to nodes:
...
The initial image builder in [`simpleimagebuilder.py`](../../analysis/cpa/simpleimagebuilder.py) is the clearest end-to-end example of this workflow.

Minimal example:

```python
from pyflow.ir.storegraph import canonicalobjects, storegraph
...
- CPA integration: [`src/pyflow/analysis/cpa`](../../analysis/cpa)
- Initial graph seeding: [`simpleimagebuilder.py`](../../analysis/cpa/simpleimagebuilder.py)
- Dump helpers: [`dumputil.py`](../../analysis/dump/dumputil.py)
- IPA memory policy: [`storegraphpolicy.py`](../../analysis/ipa/memory/storegraphpolicy.py)
