...
from pyflow.language.python.pythonoutput import PythonOutput
...
class SimpleExprGen(TypeDispatcher):
    ...
    def visitLoad(self, node):
        return (
            "<load>(%s, %r, %s)"
            % (self.process(node.expr, 24), node.fieldtype, self.process(node.name, 24))
        ), 4
    ...
    def visitDefault(self, node):
        ...
        return "<unhandled:%s>" % type(node).__name__, -1
...
class SimpleCodeGen(TypeDispatcher):
    ...
    def visitSetAttr(self, node):
        name = getConstant(node.name, str)
        stmt = "%s.%s = %s" % (
            self.seg.process(node.expr, 7),
            name,
            self.seg.process(node.value),
        )
    ...
    def visitAssign(self, node):
        if isinstance(node.expr, ast.BinaryOp) and node.expr.op in opnames.inplaceOps:
            ...
            if target != node.expr.left:
                if target == node.expr.right:
                    ...
                    temp = ast.Local(None)
                    stmt = "%s = %s" % (
                        self.seg.process(temp),
                        self.seg.process(node.expr.right),
                    )
                    self.emitStatement(stmt)

                stmt = "%s = %s" % (
                    self.seg.process(target),
                    self.seg.process(node.expr.left),
                )
                self.emitStatement(stmt)
            ...
            isinstance(node.expr, ast.Existing)
            and node.expr.object.isConstant()
            and isinstance(node.expr.object.pyobj, types.CodeType)
        ):
            # HACK
            pass
        ...
        ##			self.emitStatement(stmt)
            ...
            if (
                ...
                stmt = "%s = %s" % (
                    ", ".join([self.seg.process(lcl) for lcl in node.lcls]),
                    self.seg.process(node.expr),
                )
    ...
    def visitAssert(self, node):
        if node.message:
            stmt = "assert %s, %s" % (
                self.seg.process(node.test, 24),
                self.seg.process(node.message, 24),
            )
        else:
    ...
    def visitStore(self, node):
        stmt = "<store>(%s, %r, %s, %s)" % (
            self.seg.process(node.expr, 24),
            node.fieldtype,
            self.seg.process(node.name, 24),
            self.seg.process(node.value, 24),
        )
        self.emitStatement(stmt)
    ...
    def visitPrint(self, node):
        # Generate Python 3 print() function calls instead of Python 2 print statements
        args = []
        ...
        if node.target:
            ...
            args.append("file=%s" % target)

        if args:
            self.emitStatement("print(%s)" % ", ".join(args))
        else:
    ...
    def visitExceptionHandler(self, node):
        ...
        self.out.startBlock("except %s" % " as ".join(args))
        self(node.body)
        self.out.endBlock()
    ...
    def visitDefault(self, node):
        # Default handler for unhandled node types
        self.emitStatement("# Unhandled node type: %s" % type(node).__name__)
    ...
    def visitCode(self, node, name=None):
        ...
        glbldef = [name for name in globaldefines.keys()]
        if glbldef:
            self.out.emitStatement("global %s" % ", ".join(glbldef))
