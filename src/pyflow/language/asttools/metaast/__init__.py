...
fieldFinder = re.compile(
    r"(\w+)(?::(\w+|\([^\)]*\)))?(\?)?(\*)?"
)  # Matches name, type, optional, repeated
typeSplitter = re.compile(r"[,\s]\s*")
...
class FieldDescriptor(object):
    ...
    def __repr__(self):
        return "astfield(%r, %r, %r, optional=%r, repeated=%r)" % (
            self.name,
            self.internalname,
            self.type,
            self.optional,
...
class ClassBuilder(object):
    ...
    def addDefaultMethods(self, desc, shared):
        ...
        self.defaultFunc(
            "visitChildren",
            codegeneration.makeVisit,
            (self.name, desc),
            {"reverse": False, "shared": shared},
        )
        self.defaultFunc(
            "visitChildrenReversed",
            codegeneration.makeVisit,
            (self.name, desc),
            {"reverse": True, "shared": shared},
        )
        ...
        self.defaultFunc(
            ...
            codegeneration.makeVisit,
            (self.name, desc),
            {"reverse": False, "shared": shared, "vargs": True},
        )
        ...
        if shared:
            self.defaultFunc(
                ...
                codegeneration.makeVisit,
                (self.name, desc),
                {"reverse": False, "shared": shared, "forced": True},
            )
            self.defaultFunc(
                "visitChildrenReversedForced",
                codegeneration.makeVisit,
                (self.name, desc),
                {"reverse": True, "shared": shared, "forced": True},
            )
            self.defaultFunc(
                "visitChildrenForcedArgs",
                codegeneration.makeVisit,
                (self.name, desc),
                {"reverse": False, "shared": shared, "forced": True, "vargs": True},
            )
        else:
        ...
        if self.mutable:
            self.defaultFunc(
                "_replaceChildren",
                codegeneration.makeReplaceChildren,
                (self.name, desc, dopostinit),
            )
            self.defaultFunc(
                "replaceChildren",
                codegeneration.makeRewrite,
...
class ASTNode(object, metaclass=astnode):
