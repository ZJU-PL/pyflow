...
class Object(object):
    ...
    def initDownwardField(self, slot, fieldtype, name):
        for invoke in self.context.invokeIn.values():
    ...
    def initExistingField(self, slot, fieldtype, fieldname):
        ...
        if fieldtype == "LowLevel" and fieldname.pyobj == "type":
            ...
            values, null = analysis.externalPolicy.fieldValues(
                analysis, slot, self.name, fieldtype, fieldname
            )
        else:
            values, null = analysis.existingPolicy.fieldValues(
                analysis, slot, self.name, fieldtype, fieldname
            )

        if values:
            slot.updateValues(frozenset(values))
        if null:
            slot.markNull()
