...
`pyflow.analysis.heap` provides PyFlow's standalone heap, alias, escape, and
...
`pyflow.analysis.heap` and `pyflow.analysis.pointer` both answer points-to style
...
```python
from pyflow.analysis.heap import HeapAnalysis, HeapPolicy
...
Focused heap tests live under `tests/analysis/heap`:

```bash
pytest tests/analysis/heap
```
