...
class CFGResynthesis(object):
    ...
    def gatherInfo(self, nodes, hyperblock, entryPredicate):
        ...
        while self.pending:
            ...
            if shouldSchedule(current):
                assert all([node is not None for node in current.reverse()]), (
                    current,
                    current.reverse(),
                )
                self.depends[current] = frozenset(
                    node.defn
                    for node in current.reverse()
                    if node.isMutable() and (node.hyperblock is hyperblock)
                )
    ...
    def split(self, source):
        ...
        for block in self.blocks:
            if (
                source.canonicalpredicate
                and source.canonicalpredicate in block.predicates
            ):
        ...
        for op in self.wavefront:
            ...
            if best is None or op.canonicalpredicate is None:
                ...
                if op.canonicalpredicate in self.activePredicates:
                    if (
                        best.canonicalpredicate is not None
                        and best.canonicalpredicate not in self.activePredicates
                    ):
                        best = op
                        bestcount = opcount
...
class HighLevelAnalysis(TypeDispatcher):
    ...
    def buildCFG(self):
        ...
        for info in self.blockInfo.values():
            cfgr = CFGResynthesis()
            info.entryCFG, info.exitCFGs = cfgr.gatherInfo(
                info.entryNodes(), info.hyperblock, info.entryPredicate
            )

        # Link hyperblocks
        for info in self.blockInfo.values():
            entry = info.entryCFG
            for prev, preds in info.entryEdges.items():
    ...
    def dump(self):
        ...
        print("PRED DEPENDS")
        for pred, parent in self.predicateDepends.items():
            print(pred, parent)
        print("BLOCK OPS")
        for block, info in self.blockInfo.items():
            print("=" * 60)
            print(block)
            print(info.entryPredicate)
            print("\tEXIT EDGES")
            for prev, preds in info.exitEdges.items():
                print("\t", prev)
                for pred in preds:
                    print("\t\t", pred)
            print("\tENTRY EDGES")
