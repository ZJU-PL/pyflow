...
Migrated from Pynguin (pynguin.analyses.constants) with imports adapted to
pyflow's package structure.
"""
...
from setuptools import find_packages  # pyright: ignore[reportMissingModuleSource]
...
from pyflow.analysis.typeinfo.inference.tracing import unwrap
...
def _find_modules_with_constants(project_path: str | os.PathLike[str]) -> OrderedSet[str]:
...
def collect_static_constants(
    ...
    module_names: typing.Sequence[str] | None = None,  # noqa: UP035
) -> ConstantPool:
...
class _ConstantCollector(ast.NodeVisitor):
    ...
    def visit_Constant(self, node: ast.Constant):  # noqa: N802
        if type(node.value) in typing.get_args(ConstantTypes):
            self._pool.add_constant(node.value)  # type: ignore[arg-type]  # pyright: ignore[reportArgumentType]
