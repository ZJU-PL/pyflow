...
if TYPE_CHECKING:
    from pyflow.analysis.typeinfo.core.typesystem import ProperType, TypeVarType
...
def bind_generics(
    ...
    Examples:
        >>> T = TypeVarType("T")
        >>> U = TypeVarType("U")
        >>> ti = ClassDescriptor(int)
        >>> binding = bind_generics([T, U], [Instance(ti), Instance(ti)])
        >>> binding.mapping
        {'T': Instance(ClassDescriptor(int)), 'U': Instance(ClassDescriptor(int))}
    """
