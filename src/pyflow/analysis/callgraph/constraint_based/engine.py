...
class ConstraintCallGraphBuilder(
    ...
    def _new_container(self, kind: str, scope: ScopeInfo, node: ast.AST) -> AbstractValue:
        line = getattr(node, "lineno", -1)
        col = getattr(node, "col_offset", -1)
        token = f"{kind}:{scope.name}@{line}:{col}:{self._container_counter}"
        self._container_counter += 1
        return make_container(token)
