    ...
    **Color Scheme:**
    ...
    def _write_control_relations(self, f):
        ...
        Args:
            f: File handle to write to
        """
        for title, attr, arrow in [
            ("Control Dependencies", "dependents", "->"),
    ...
    def dump_dot(self, output_file: str, function_name: str = ""):
        ...
        graph.set_label(
            f"Control Dependence Graph{f' for {function_name}' if function_name else ''}"
        )
        ...
        with open(output_file, "w") as f:
            f.write(
                f"// Control Dependence Graph{f' for function: {function_name}' if function_name else ''}\n"
            )
    ...
    def _add_dot_node(self, graph, cfg_node, cdg_node):
        ...
        graph.add_node(
            pydot.Node(
                ...
                shape="box",
                style="filled",
                fillcolor=self._get_color(cfg_node),
                fontsize=10,
            )
        )
    ...
    def _add_dot_edge(self, graph, edge):
        ...
        graph.add_edge(
            pydot.Edge(
                f"node_{edge.source.node_id}",
                f"node_{edge.target.node_id}",
                label=edge.label or "control",
                color=self._get_color(edge.label, "edges"),
                fontsize=8,
            )
        )
