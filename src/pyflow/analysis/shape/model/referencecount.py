...
class ReferenceCountManager(object):
    ...
    def split(self, rc, accessedCallback):
        ...
        for slot, count in rc.counts.items():
    ...
    def merge(self, a, b):
        ...
        newrc = {}
        if a:
            newrc.update(a.counts)
        if b:
            newrc.update(b.counts)
...
class ReferenceCount(object):
    ...
    def __repr__(self):
        rc = ["%s=%s" % p for p in self.counts.items()]
    ...
    def forget(self, sys, kill):
        newcounts = {}
        for slot, count in self.counts.items():
            if slot not in kill:
