...
class Configuration(object):
    ...
    def __repr__(self):
        return "conf(object=%r, entry=%r, current=%r, ex=%r)" % (
            self.object,
            self.entrySet,
            self.currentSet,
    ...
    def rewrite(
        ...
        if object_ is noChange:
            object_ = self.object
        if region is noChange:
            region = self.region
        if entrySet is noChange:
            entrySet = self.entrySet
        if currentSet is noChange:
            currentSet = self.currentSet
        if externalReferences is noChange:
            externalReferences = self.externalReferences
        if allocated is noChange:
            allocated = self.allocated

        return sys.canonical.configuration(
            object_, region, entrySet, currentSet, externalReferences, allocated
        )
