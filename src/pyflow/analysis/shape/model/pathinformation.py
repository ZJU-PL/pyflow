...
class EquivalenceClass(object):
    ...
    def copy(self, lut, kill, keepHits=False, keepMisses=False):
        if self in lut:
            ...
            if self.attrs:
                for attr, next in self.attrs.items():
    ...
    def remap(self, lut, mapping):
        if self in lut:
            ...
            if self.attrs:
                for slot, next in self.attrs.items():
                    while next.forward is not None:
                        next = next.forward
                    ...
                    assert not newslot or not newslot.isExtendedParameter(), (
                        newslot,
                        slot in mapping,
                    )
    ...
    def ageExtended(self, canonical):
        if self.attrs is not None:
            newAttr = {}
            for slot, eq in self.attrs.items():
                while eq.forward is not None:
                    eq = eq.forward
            ...
            if self.attrs is not None:
                for slot, next in self.attrs.items():
                    ...
                    if accessed and not extended:
                        ...
                        newNext, newPure = next._splitHidden(
                            extendedParameters, sharedEq, accessedCallback, lut, noKill
                        )
                        pure &= newPure
...
class PathInformation(object):
    ...
    def containsExtended(self):
        if self.root.attrs is not None:
            for slot in self.root.attrs.keys():
                if slot.isExtendedParameter():
                    return True
        return False
