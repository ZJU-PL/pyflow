...
class GetLocals(TypeDispatcher):
    ...
    def default(self, node):
        # Some children (e.g., program.Object) are not AST nodes; skip them
...
class ShapeConstraintBuilder(TypeDispatcher):
    ...
    def post(self, node):
        # Always record the post mapping
    ...
    def process(self, node):
        ...
        self.functionLocalSlots[node] = frozenset(
            [self.sys.canonical.localSlot(lcl) for lcl in self.functionLocals[node]]
        )
        self.functionLocalExprs[node] = frozenset(
            [self.sys.canonical.localExpr(lcl) for lcl in self.functionLocalSlots[node]]
        )
    ...
    # Convenience wrapper: forget a single expression's slot
    def forget(self, expr):
        # Ignore if no expression provided
        if expr is None:
            return
        # Expressions in this analysis expose the underlying slot via 'slot'
        if hasattr(expr, "slot") and expr.slot is not None:
            self.forgetAll((expr.slot,))

    # Insert a copy/move of the abstract state from one program point to another
    def copy(self, srcPoint, dstPoint):
        constraint = constraints.CopyConstraint(self.sys, srcPoint, dstPoint)
        self.constraints.append(constraint)
    ...
    def makeCallerArgs(self, node, targets):
        ...
        from pyflow.util.python import calling as _calling

        callerargs = _calling.CallerArgs(selfarg, args, kwds, vargs, kargs, returnargs)
        return callerargs
    ...
    def _makeCalleeParams(self, node):
        ...
        calleeparams = _calling.CalleeParams(
            selfparam, params, paramnames, defaults, vparam, kparam, returnparams
        )
        return calleeparams
    ...
    def computeTransfer(self, callerargs, calleeparams):
        ...
        info = _calling.callStackToParamsInfo(
            calleeparams,
            selfArg,
            numArgs,
            vargsUncertain,
            callerargs.kwds.keys(),
            False,
        )
        return info
    ...
    def visitSwitch(self, node):
        ...
        if tOut or fOut:
            self.advance()
            if tOut:
                self.copy(tOut, self.current)
            if fOut:
                self.copy(fOut, self.current)
    ...
    def visitCode(self, node):
        ...
        if self.debug:
            ...
            print(self.codeReturnPoint(node))
        ...
        self.forgetAll(lcls, self.codeReturnPoint(node))
