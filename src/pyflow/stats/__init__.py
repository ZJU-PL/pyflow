...
classes = "user", "interp", "runtime", "primitive"
...
def opRatios(collect, classOK):
    ...
    if classOK:
        for cls in classes:
            ...
            builder.row(
                cls,
                codeCount,
                100.0 * codeCount / totalCode,
                contextCount,
                100.0 * contextCount / totalContexts,
    ...
    builder.row(
        ...
        totalContexts,
        100.0,
        float(totalContexts) / totalCode,
    )

    if not classOK:
        builder.rewrite(0, 2, 4)
    ...
    for cls in classes:
        ...
        if classOK:
            builder.row(
                cls,
                codeCount,
                ratio(100.0 * codeCount, totalCode),
                contextCount,
                ratio(100.0 * contextCount, totalContexts),
                ratio(contextCount, codeCount),
            )
...
def opPieChart(collect):
    ...
    collect.subfigures.append(
        r"\subfloat[\label{fig:%s-op-piechard}]{\includegraphics[width=0.45\columnwidth]{op-piechart}"
        % collect.name
    )
...
def generateIndex(collect):
    ...
    name = collect.name
    print(r"\caption{%s Statistics}" % (name[0].upper() + name[1:]), file=f)
...
def contextStats(compiler, prgm, name, classOK=False):
    ...
    # Histogram
    if False:
        for count in sorted(collect.counts.keys()):
            print(count)
            for cls, num in collect.counts[count].items():
                print("\t", cls, num)
            print()

    tw = collections.defaultdict(lambda: 0)
    tc = collections.defaultdict(lambda: 0)
    for xtype, count in collect.access.items():
    ...
    tow = 0
    otc = 0
    for t, count in collect.taccess.items():
        rel = tw[t] / float(tc[t])
        print(t, count, rel)
        paw += rel
        tow += count
    ...
    if False:
        for cls, lut in collect.opCount.items():
            print(cls)
            for opT, num in lut.items():
                print("\t", opT, num)
            print()
        print()
