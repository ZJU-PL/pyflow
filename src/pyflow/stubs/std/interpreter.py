...
def makeInterpreterStubs(collector):
    ...
    func_globals_attr = collector.compiler.slots.uniqueSlotName(
        types.FunctionType.__dict__["__globals__"]
    )
    ...
    def interpreterStoreGlobal(function, name, value):
        globalDict = function.__globals__
...
""" % {
    ...
    for op in opnames.unaryPrefixLUT.values():
    ...
    for op in opnames.binaryOps:
        if op in opnames.forward and op in opnames.reverse:
            ...
            try:
                ...
                pass  # Some operations might not have operator equivalents

    # Generate inplace binary operations
    for op in opnames.inplaceOps:
        base_op = opnames.inplaceFallback[op]
        if base_op in opnames.forward and base_op in opnames.reverse:
            iattr = opnames.inplace[base_op]
            attr = opnames.forward[base_op]
            rattr = opnames.reverse[base_op]
            f = simpleInplaceBinaryOp("interpreter%s" % iattr, iattr, attr, rattr)
