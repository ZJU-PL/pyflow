...
def readLeda(lines):
    ...
    lines = iter(lines)
    for i in range(3):
        lines.__next__()  # skip header lines
    G = graph()

    n = graphNum(lines.__next__())  # number of vertices
    for i in range(n):
        vertex(G, i + 1)
        lines.__next__()  # skip LEDA data

    m = graphNum(lines.__next__())  # number of edges
    for i in range(m):
        words = lines.__next__().split()
        if len(words) < 4:
