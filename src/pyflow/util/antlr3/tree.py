...
from pyflow.util.antlr3.constants import UP, DOWN, EOF, INVALID_TOKEN_TYPE
from pyflow.util.antlr3.recognizers import BaseRecognizer
from pyflow.util.antlr3.streams import IntStream
from pyflow.util.antlr3.tokens import CommonToken, Token, INVALID_TOKEN
from pyflow.util.antlr3.exceptions import (
...
class BaseTree(Tree):
    ...
    def sanityCheckParentAndChildIndexes(self, parent=None, i=-1):
        ...
        if i != self.childIndex:
            raise ValueError(
                "child indexes don't match; expected %d found %d" % (i, self.childIndex)
            )
...
class TreeParser(BaseRecognizer):
    ...
    def getErrorHeader(self, e):
        ...
        return self.getGrammarFileName() + ": node from %sline %s:%s" % (
