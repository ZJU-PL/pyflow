...
class LeakDetector(Detector):
    name = "leak"
    description = (
        "Detect resource leaks (file handles) and scope leaks (escaping objects)."
    )
        ...
        for fname, src in session.sources_by_name.items():
            ...
            for lineno in visitor.open_calls:
                if lineno not in resource_leaks and lineno in visitor.unclosed:
                    resource_leaks[lineno] = (
                        "File handle opened without context manager/close()."
                    )
    ...
    def _scope_leak_reports(self, session: AnalysisSession) -> List[Issue]:
        ...
        for obj, info in objects.items():
            ...
            if getattr(info, "globallyVisible", False) or getattr(
                info, "externallyVisible", False
            ):
                continue
    ...
    def _analyze_escape_reason(
        ...
        if hasattr(info, "assignedToGlobal") and getattr(
            info, "assignedToGlobal", False
        ):
                ...
                if hasattr(code, "codeName"):
                    ...
                    if func_name:
                        ...
                        try:
                            ...
                            for var_name, alias_info in aliases.items():
                                if hasattr(alias_info, "escapes") and getattr(
                                    alias_info, "escapes", False
                                ):
