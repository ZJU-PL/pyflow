...
TODO: currently, this file almost implements its own dataflow analysis (although it uses the analysis session).
Maybe we can decide how to better reuse the existing analyses inside pyflow.
Maybe we can create a new taint engien (the current one seem to work well on the micro-benchmark "sast-python3")
"""
...
class _LocalTaintAnalyzer(ast.NodeVisitor):
    ...
    def visit_Match(self, node: ast.Match):
        ...
        Key semantics:
        - Only propagate taint through actual data flow, not syntactic bindings
        - MatchAs with a name: the name is bound to the subject value, so it inherits subject taint
        - MatchClass: bindings are matched against attributes of the subject, not the subject itself
        - MatchMapping/MatchSequence: bindings are matched against elements/keys of the subject
        - Literal patterns don't propagate taint (they're matching against constants)
        """
        subject_is_tainted = self._expr_is_tainted(node.subject)

        for case in node.cases:
            # Collect all bindings in this case pattern
            bindings = self._collect_match_bindings(case.pattern)

            # Only propagate taint to bindings that directly capture the subject
            # MatchAs patterns like `case X as name` bind `name` to the subject value
            # Other patterns bind to parts of the subject, not the subject itself
            for name in bindings:
                # Check if this binding directly captures the subject
    ...
    def _binding_captures_subject(self, pattern: ast.pattern, binding_name: str, subject_is_tainted: bool) -> bool:
        """Check if a binding name in a pattern directly captures the subject value.

        Only MatchAs patterns with a name directly capture the subject.
        Other patterns (MatchClass, MatchMapping, MatchSequence) capture
        parts of the subject (attributes, elements, values), not the subject itself.
        """
        if not subject_is_tainted:
            return False

        if isinstance(pattern, ast.MatchAs):
            # MatchAs captures the subject (or nested pattern result) into name
        ...
        # Other pattern types don't directly capture the subject
        return False
    ...
    def visit_Assign(self, node: ast.Assign):
        ...
        for target in node.targets:
            if isinstance(target, ast.Name):
                ...
                # Destructuring assignment: a, b, c = iterable
                # Only taint target variables that receive tainted values from specific positions
                if value_is_source:
                    for elt in target.elts:
                        if isinstance(elt, ast.Name):
                            self.tainted.add(elt.id)
                elif value_is_tainted:
                    # For destructuring, we need to check which positions are tainted
                    if isinstance(node.value, (ast.Tuple, ast.List, ast.Set)):
                        # Direct unpacking: a, b, c = [x, y, z]
                        # Only taint targets whose corresponding source elements are tainted
                        ...
                        # Unpacking from a potentially tainted container
                        # Conservative: taint all targets since we can't know which elements are tainted
                        for elt in target.elts:
                            if isinstance(elt, ast.Name):
                                self.tainted.add(elt.id)
                    else:
                        # For other cases (function returns, etc.), be conservative
                        for elt in target.elts:
                            if isinstance(elt, ast.Name):
                                self.tainted.add(elt.id)
        self.generic_visit(node)
    ...
    def visit_AnnAssign(self, node: ast.AnnAssign):
        """Handle annotated assignments (e.g., x: int = 5 or x: List[int] = [...]).

        Type annotations should not affect taint tracking - only the runtime value matters.
        """
        if not isinstance(node.target, ast.Name):
            # Handle subscript or attribute targets with annotations
            self.generic_visit(node)
            return
        ...
        if node.value is None:
            # Variable declaration with annotation only: x: int
            # No value assigned, don't mark as tainted
            self.generic_visit(node)
            return
    ...
    def visit_Delete(self, node: ast.Delete):
        """Track del operations to remove taint from containers and variables."""
        for target in node.targets:
            if isinstance(target, ast.Name):
                # Deleting a variable removes it from tainted set
                self.tainted.discard(target.id)
                # Also clear any container taint associated with this name
                self._clear_container_taint(target.id)
            elif isinstance(target, ast.Attribute):
                # Deleting obj.attr - remove the attribute taint
                base, attr = self._attribute_base_and_attr(target)
                if base and attr:
                    base_key = self._alias_key(base)
                    attrs = self.tainted_attrs.get(base_key)
                ...
                # Deleting container[key] - remove taint from that specific key
                base = self._subscript_base_name(target.value)
                key = self._subscript_key(target.slice)
                if base:
                    self._clear_container_key(base, key)
    ...
    def _expr_is_tainted(self, expr: ast.AST) -> bool:
        ...
        if isinstance(expr, ast.BinOp):
            # For binary operations (other than comparisons), if either operand is tainted,
            # the result could be tainted (e.g., arithmetic on tainted values)
            return self._expr_is_tainted(expr.left) or self._expr_is_tainted(expr.right)
        if isinstance(expr, ast.BoolOp):
            # BoolOp (or, and) uses short-circuit evaluation:
            # - a or b: returns a if truthy, else b
            # - a and b: returns a if falsy, else b
            # Taint only propagates if the ACTUAL RETURNED value is tainted
            # This requires knowing which branch is taken, which we can't always determine
            # conservatively: if any operand could be returned AND is tainted, mark as tainted
            for value in expr.values:
                if self._expr_is_tainted(value):
                    return True
            return False
        if isinstance(expr, ast.UnaryOp):
            # Unary operations: 'not' always returns bool (cleans taint)
            # Other unary ops (-, +, ~) propagate taint from operand
            if isinstance(expr.op, ast.Not):
                # 'not x' always returns True or False, never the original value
                # Therefore, it does NOT propagate taint to the result
                return False
            return self._expr_is_tainted(expr.operand)
        if isinstance(expr, ast.Compare):
            # Comparison results are always boolean, not the original values
            return False
