...
from ..core.support import _concrete, _unique_values
...
class _ValueMixin:
    def _binary(
        ...
        if isinstance(left, _PathValue) and isinstance(operator, ast.Div):
            return _PathValue(posixpath.join(left.concrete, self._to_string(right).concrete))
        if (
            ...
            if isinstance(operator, ast.BitAnd):
                return _SetValue(
                    [
                        ...
                        if any(
                            self._equals(value, candidate).concrete for candidate in right.values
                        )
                    ]
                )
            if isinstance(operator, ast.Sub):
        ...
        if isinstance(left, _InstanceValue):
            ...
            for kind, name in methods.items():
                if isinstance(operator, kind):
                    method_with_owner = self._method_with_owner(left.class_value, name)
        ...
        if (
            ...
            return _StringValue(left.concrete + right.concrete, left.symbolic + right.symbolic)
        ...
        if isinstance(operator, ast.Mult) and isinstance(right, (_StringValue, _ListValue)):
        ...
        if isinstance(operator, ast.Mod):
            ...
            return _IntValue(lhs.concrete % rhs.concrete, lhs.symbolic - rhs.symbolic * quotient)
        ...
        for operator, right in pairs:
            ...
            if isinstance(operator, (ast.Is, ast.IsNot)):
                concrete = (
                    ...
                    if isinstance(left, _BuiltinFunction) and isinstance(right, _BuiltinFunction)
            ...
            if isinstance(left, _EnumMember) and isinstance(right, _EnumMember):
                if not isinstance(operator, (ast.Eq, ast.NotEq)):
                    raise UnsupportedSyntaxError("enum members only support equality comparison")
            ...
            if isinstance(left, _InstanceValue):
                ...
                if method_with_owner is not None:
                    method, owner = method_with_owner
                    equality = self._truthy(self._call_method(method, owner, left, [right], {}))
                    concrete, symbolic = equality.concrete, equality.symbolic
            ...
            if left is None or right is None:
                if not isinstance(operator, (ast.Eq, ast.NotEq)):
                    raise UnsupportedSyntaxError("None only supports equality comparison")
    ...
    def _assign(self, target: ast.expr, value: Any) -> None:
        ...
        if isinstance(target, ast.Subscript):
            ...
            if isinstance(container, _InstanceValue):
                method_with_owner = self._method_with_owner(container.class_value, "__setitem__")
                if method_with_owner is not None:
                    method, owner = method_with_owner
                    key = (
            ...
            if isinstance(target.slice, ast.Slice):
                ...
                container.values[self._slice_indices(target.slice, len(container.values))] = list(
                    self._iter_values(value)
                )
                return
        ...
        if isinstance(target, ast.Attribute):
            ...
            if isinstance(instance, _InstanceValue):
                setter = self._property_setter_with_owner(instance.class_value, target.attr)
        ...
        if isinstance(target, (ast.Tuple, ast.List)):
            ...
            if len(starred) > 1:
                raise UnsupportedSyntaxError("unpacking assignment has multiple starred targets")
            if not starred:
                if len(target.elts) != len(values):
                    raise UnsupportedSyntaxError("unpacking assignment has mismatched length")
            ...
            if trailing_count:
                for element, item in zip(target.elts[-trailing_count:], values[-trailing_count:]):
                    self._assign(element, item)
            return
    ...
    def _assign_name(self, name: str, value: Any) -> None:
        if name in self._global_names:
            ...
            if self._closure_env is None or name not in self._closure_env:
                raise ConcolicError(f"nonlocal name {name!r} has no enclosing binding")
    ...
    def _delete(self, target: ast.expr) -> None:
        ...
        if isinstance(target, ast.Subscript):
            container = self._evaluate(target.value)
            if isinstance(target.slice, ast.Slice) and isinstance(container, _ListValue):
                del container.values[self._slice_indices(target.slice, len(container.values))]
                return
            index = self._evaluate(target.slice)
            if isinstance(container, _ListValue):
    ...
    def _subscript(self, container: Any, slice_node: ast.expr) -> Any:
        if isinstance(container, _InstanceValue):
            method_with_owner = self._method_with_owner(container.class_value, "__getitem__")
            if method_with_owner is not None:
                method, owner = method_with_owner
                key = (
        ...
        if isinstance(container, (_ListValue, _TupleValue, _NamedTupleValue, _SetValue)):
    ...
    def _slice(self, container: Any, slice_node: ast.Slice) -> Any:
        ...
        if isinstance(container, _StringValue):
            ...
            concrete = container.concrete[self._slice_indices(slice_node, len(container.concrete))]
        ...
        if isinstance(container, _BytesValue):
            return _BytesValue(
                container.concrete[self._slice_indices(slice_node, len(container.concrete))]
            )
        if isinstance(container, _ListValue):
            return _ListValue(
    ...
    def _slice_indices(self, node: ast.Slice, _length: int) -> slice:
        ...
        start = (
            self._as_int(self._evaluate(node.lower)).concrete if node.lower is not None else None
        )
        stop = self._as_int(self._evaluate(node.upper)).concrete if node.upper is not None else None
    ...
    def _string_index(self, value: _StringValue, index: _IntValue) -> _StringValue:
        ...
        if index.concrete < 0:
            return _StringValue(concrete, self._z3.StringVal(concrete))
        return _StringValue(concrete, self._z3.SubString(value.symbolic, index.symbolic, 1))
    ...
    def _as_iterator(self, value: Any) -> _IteratorValue:
        ...
        if isinstance(value, _BytesValue):
            return _SequenceIteratorValue(tuple(self._literal(item) for item in value.concrete))
        if isinstance(value, _StringValue):
            return _SequenceIteratorValue(
                tuple(_StringValue(char, self._z3.StringVal(char)) for char in value.concrete)
            )
        if isinstance(value, _DictValue):
            return _SequenceIteratorValue(tuple(self._literal(key) for key in value.values))
        ...
        if isinstance(value, _InstanceValue):
            ...
            if method_with_owner is not None:
                method, owner = method_with_owner
                return self._as_iterator(self._call_method(method, owner, value, [], {}))
    ...
    def _resume_iterator(
        ...
        if self._resume_steps > self._max_resume_steps:
            raise ConcolicError(
                "iterator execution exceeded --max-resume-steps " f"({self._max_resume_steps})"
            )
    ...
    def _iter_values(self, value: Any) -> tuple[Any, ...]:
        ...
        values: list[Any] = []
        while True:
            outcome = self._resume_iterator(iterator, _ResumeOperation(_ResumeKind.NEXT))
        ...
        def visit(index: int) -> None:
            if index == len(generators):
                if isinstance(expression, tuple):
                    output.append((self._evaluate(expression[0]), self._evaluate(expression[1])))
                else:
    ...
    def _to_int(self, value: Any) -> _IntValue:
        ...
        if isinstance(value, _StringValue):
            try:
                ...
                raise ConcolicError(f"invalid literal for int(): {value.concrete!r}") from error
            if value.concrete.startswith("-"):
                symbolic = -self._z3.StrToInt(
                    self._z3.SubString(value.symbolic, 1, self._z3.Length(value.symbolic) - 1)
                )
            else:
    ...
    def _to_float(self, value: Any) -> _FloatValue:
        ...
        if isinstance(value, _BoolValue):
            return _FloatValue(float(value.concrete), self._z3.If(value.symbolic, 1.0, 0.0))
        if isinstance(value, _StringValue):
            try:
            ...
            if not math.isfinite(concrete):
                raise UnsupportedSyntaxError("float() does not support non-finite values")
    ...
    def _to_string(self, value: Any) -> _StringValue:
        ...
        return _StringValue(str(_concrete(value)), self._z3.StringVal(str(_concrete(value))))

    def _format_value(self, value: Any, specification: str) -> _StringValue:
        if isinstance(value, _InstanceValue):
            method_with_owner = self._method_with_owner(value.class_value, "__format__")
            if method_with_owner is not None:
                method, owner = method_with_owner
                return self._to_string(
                    self._call_method(
                        ...
                        owner,
                        value,
                        [_StringValue(specification, self._z3.StringVal(specification))],
                        {},
                    )
                )
        try:
    ...
    def _to_bytes(self, value: Any) -> _BytesValue:
        ...
        if isinstance(value, (_ListValue, _TupleValue, _RangeValue)):
            return _BytesValue(bytes(self._as_int(item).concrete for item in value.values))
        raise UnsupportedSyntaxError("bytes() requires bytes or an iterable of integers")
    ...
    def _range(self, args: list[Any]) -> _RangeValue:
        ...
        return _RangeValue(
            tuple(
                ...
                for offset, item in enumerate(range(start.concrete, stop.concrete, step.concrete))
            )
        )
    ...
    def _aggregate(self, name: str, args: list[Any]) -> _IntValue | _FloatValue:
        ...
        for concrete, symbolic in reals[1:]:
            choose = (
                selected_symbolic >= symbolic if name == "max" else selected_symbolic <= symbolic
            )
            selected_concrete = (
            ...
            selected_symbolic = self._z3.If(choose, selected_symbolic, symbolic)
        return _FloatValue(selected_concrete, selected_symbolic)

    def _heap_entries(self, values: list[Any]) -> list[tuple[Any, int, Any]]:
        entries = [(_concrete(value), index, value) for index, value in enumerate(values)]
        try:
    ...
    def _copy_value(self, value: Any, *, deep: bool) -> Any:
        copy_value = (lambda item: self._copy_value(item, deep=True)) if deep else lambda item: item
        ...
        if isinstance(value, _DictValue):
            return _DictValue({key: copy_value(item) for key, item in value.values.items()})
        if isinstance(value, _InstanceValue):
