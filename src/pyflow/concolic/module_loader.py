        ...
        if isinstance(statement, ast.Import):
            for alias in statement.names:
                ...
                if alias.name == "importlib":
                    module.globals[alias.asname or "importlib"] = _ImportlibModule(
                        path, cache
                    )
                    continue
                ...
                if alias.name in _SUMMARY_MODULES:
                    module.globals[alias.asname or alias.name] = _SummaryModule(
                        alias.name
                    )
                    continue
            ...
            if statement.level == 0 and statement.module == "importlib":
                for alias in statement.names:
                    if alias.name == "import_module":
                        module.globals[alias.asname or alias.name] = _ImportlibFunction(
                            path, cache
                        )
                continue
            ...
            imported = _resolve_local_module(
                path, statement.module, cache, statement.level
            )
            if imported is None:
                continue
            for alias in statement.names:
                if alias.name == "*":
                    ...
                    module.classes.update(imported.classes)
                    module.globals.update(imported.globals)
                elif alias.name in imported.functions:
                    module.globals[alias.asname or alias.name] = _FunctionValue(
                        imported.functions[alias.name], {}, imported
                    )
                elif alias.name in imported.classes:
                    module.classes[alias.asname or alias.name] = imported.classes[
                        alias.name
                    ]
                elif alias.name in imported.globals:
                    module.globals[alias.asname or alias.name] = imported.globals[
                        alias.name
                    ]
                else:
                    resolved = _import_local_module(
                        imported.path, alias.name, cache
                    )
                    if resolved is not None:
                        _, child = resolved
            ...
            for alias in statement.names:
                imported = _resolve_local_module(
                    path, alias.name, cache, statement.level
                )
                if imported is not None:
                    module.globals[alias.asname or alias.name] = imported
...
def _static_literal(node: ast.expr, bindings: dict[str, Any] | None = None) -> Any:
    ...
    if isinstance(node, ast.BinOp):
        ...
        operation = next(
            (
                callback
                for kind, callback in operations.items()
...
def _static_namedtuple(node: ast.expr) -> _NamedTupleClass | object:
    ...
    if isinstance(field_specification, str):
        fields = tuple(
            field
            for field in field_specification.replace(",", " ").split()
            if field
        )
