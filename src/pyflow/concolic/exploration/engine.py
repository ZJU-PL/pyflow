...
from ..core.support import _concrete, _input_key, _valid_input
...
def explore_file(
    ...
    *,
    ...
    while pending and counters.executions < max_iterations:
        ...
        post_inputs = tuple(_concrete(executor.env[name]) for name in parameter_names)
        ...
        for index, branch in enumerate(path_constraints):
            path_target: tuple[tuple[str, bool], ...] = tuple(
                prior.key() for prior in path_constraints[:index]
            ) + ((str(branch.expression.sexpr()), not branch.taken),)
            if path_target in observed_path_prefixes or path_target in queued_paths:
                continue
            queued_paths.add(path_target)
            solved = _solve_path(
                z3,
                parameter_names,
                initial,
...
def _solve_path(
    ...
    return _SolveResult(
        tuple(
            _model_value(model, z3, name, kind) for name, kind in zip(parameter_names, input_kinds)
        )
    )
...
def _model_value(model: Any, z3: Any, name: str, kind: Any) -> Any:
    if isinstance(kind, list):
        return [_model_value(model, z3, f"{name}_{index}", item) for index, item in enumerate(kind)]
    if isinstance(kind, dict):
        return {key: _model_value(model, z3, f"{name}_{key}", item) for key, item in kind.items()}
