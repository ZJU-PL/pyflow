...
class FileCompressor:
    ...
    def compress_file(self, filename, compression_level=9):
        ...
        # Fixed command execution - no shell=True and proper argument handling
        try:
            with open(f"{output_path}.gz", 'wb') as out_file:
                subprocess.run(
                    ['gzip', f'-{compression_level}', '-c', filename],
                    stdout=out_file,
            ...
            raise RuntimeError(f"Compression failed: {e.stderr.decode().strip()}")
