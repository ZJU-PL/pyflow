...
app = Flask(__name__)
app.secret_key = 'supersecretkey'  # In production, use a proper secret key from environment variables
...
def add_review():
    ...
    try:
        ...
        if len(review) > 1000:
            return "Review too long", 400
        
        conn = get_db_connection()
        # Check book exists
        book = conn.execute("SELECT 1 FROM books WHERE id = ?", (book_id,)).fetchone()
        if not book:
            conn.close()
