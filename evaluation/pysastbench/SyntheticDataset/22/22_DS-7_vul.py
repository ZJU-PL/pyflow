...
app = Flask(__name__)
app.config['LOG_DIR'] = '/var/log/app_logs'
...
class LogManager:
    ...
    def rotate_logs(self):
        for log_file in os.listdir(app.config['LOG_DIR']):
            ...
            if os.path.getsize(file_path) > app.config['MAX_LOG_SIZE']:
                timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                new_name = f"{log_file}.{timestamp}.bak"
                os.rename(file_path, os.path.join(app.config['LOG_DIR'], new_name))

    def get_log_file(self, filename, max_lines=1000):
        # Vulnerable path traversal - filename concatenated without validation
...
def list_log_files():
    try:
        logs = [f for f in os.listdir(app.config['LOG_DIR']) 
               if os.path.isfile(os.path.join(app.config['LOG_DIR'], f))]
        return {'logs': logs}
    except Exception as e:
        abort(500, description=str(e))
