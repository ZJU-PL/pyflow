...
class DocumentManager:
    ...
    def get_document(self, doc_request):
        # Secure version - validates the final path is within DOCUMENT_ROOT
        filename = secure_filename(doc_request)
        if not filename:
            return None
            
        doc_path = os.path.join(app.config['DOCUMENT_ROOT'], filename)
        
        # Additional security check to prevent path traversal
        doc_path = os.path.normpath(doc_path)
        if not doc_path.startswith(os.path.abspath(app.config['DOCUMENT_ROOT']) + os.sep):
            return None
