...
class TemplateHandler(tornado.web.RequestHandler):
    def post(self):
        ...
        def safe_render(content):
            ...
            try:
                # First escape any HTML/JS to prevent XSS
                escaped_content = html.escape(content)
                # Then allow basic formatting with newlines
                rendered = escaped_content.replace('\n', '<br>')
                return rendered
            except Exception as e:
                return f"Error: {html.escape(str(e))}"
        
        rendered = safe_render(template_content)
        self.write(f'''
        <h2>Rendered Template: {html.escape(template_name)}</h2>
